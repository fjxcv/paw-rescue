...
演示账号（由 `seed_demo` 创建，无需 `createsuperuser`）：
...
新成员三步启动：`migrate` → `seed_demo` → `seed_test_data`（首页统计与最新动态数据更丰富）。
...
- 拉取代码后执行 `migrate`，再依次运行 `seed_demo` 与 `seed_test_data` 填充演示数据。
- 演示管理员账号：`admin` / `admin12345`（由 `seed_demo` 创建）。
